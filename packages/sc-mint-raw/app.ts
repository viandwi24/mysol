...
export async function main() {
  ...
  const configYml = fs.readFileSync(CONFIG_FILE_PATH, "utf8");
  const keypairPath = await yaml.parse(configYml).keypair_path;
  const wallet = createKeypairFromFile(keypairPath);

  const admin = createKeypairFromFile(
    path.join(__dirname, "./../../configs/keypairs/u1.json")
  );
